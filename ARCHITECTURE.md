...
5. Проверить соответствие callback-а найденному invoice, включая сумму и валюту.
6. Вызвать `ConfirmPaymentSucceeded`.
7. Зафиксировать транзакцию.
8. Вернуть `OK{InvId}`.

`HandleRobokassaResult` не должен самостоятельно менять статус платежа, писать историю статуса или создавать outbox event. Он отвечает за протокол Robokassa, проверку входящего callback-а и управление транзакцией webhook-а.
...
2. Если платёж уже `succeeded`, ничего не менять и вернуть успешный результат.
3. Проверить допустимость перехода статуса.
4. Перевести платёж в `succeeded`.
5. Записать историю статуса.
6. Создать outbox event `PaymentSucceeded`.

`ConfirmPaymentSucceeded` является единственным местом, где выполняется переход платежа в `succeeded`. Это сохраняет одинаковую идемпотентность и одинаковые side effects для callback-а Robokassa и reconciliation.
